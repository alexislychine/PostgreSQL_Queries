...
DBVER=`PGPASSWORD=$MYPASS $PSQLCL -c "select version()" | sed -n '3 p'|awk  '{print $2}'|egrep -o '[0-9]{2,}'`
...
SQL12="SELECT substring(query, 1, 50) AS short_query,
              round(total_plan_time::numeric, 2) AS total_plan_time,
              calls,
              round(mean_plan_time::numeric, 2) AS mean,
              round((100 * total_plan_time /
              sum(total_plan_time::numeric) OVER ())::numeric, 2) AS percentage_cpu
...
#Top 10 short queries consuming CPU for Postgres 13 and higher
SQL20="SELECT  substring(query, 1, 50) AS query,
round((total_plan_time+total_exec_time)::numeric, 2) AS total_plan_time,
calls,
round((mean_plan_time+mean_exec_time)::numeric, 2) AS mean,
...
#Top 10 Read Queries for Postgres 13 and higher
SQL21="SELECT
  left(query, 50) AS short_query
  ,round((total_plan_time+total_exec_time)::numeric, 2) AS total_plan_time
  ,calls
...
echo `PGPASSWORD=$MYPASS $PSQLCL -c "SET statement_timeout='60s' ; SET idle_in_transaction_session_timeout='60s'; select version()" | sed -n '3 p'|awk  '{print $2}'  `  >>$html
echo "<br>" >> $html
